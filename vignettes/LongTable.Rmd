...
the way data is mapped from your raw data, as a single large `data.frame`,
to the various slots of a `LongTable` object.
...
define a basic interface for any classes inheriting from it.

This object is not important for general users, but may be useful for other
developers who want to map from some rawdata to some `S4` class. In this case,
any derived data mapper should inherit from the `DataMapper` abstract class. 
Only slot is defined by default, a `list` or `List` in the `@rawdata` slot. An
accessor method, `rawdata(DataMapper)`, is defined to assign and retrieve
the raw data from your mapper object.

## The LongTableDataMapper Class

The `LongTableDataMapper` class is the first concrete sub-class of a `DataMapper`.
It is the object which defines how to go a single `data.frame` or `data.table` 
of raw experimental data, to a properly formatted and valid `LongTable` object. 
This is accomplished by defining various maps, which let the the user decide
which columns from `rawdata` should go into which slots of the object.

Additionally, a helper method has been included, `guessMapping`, which will
try to determine which dimensions of a `LongTable`s `rawdata` should be assigned
...
database of drug combination sensitivity experiments which we created and 
released (Seo **et al.**, 2019).
...
# First we put out data in the `LongTableDataMapper`
LTdataMapper <- LongTableDataMapper(rawdata=merckDT)

# Then we can test our hypothesis, subset=FALSE means we don't remove mapped 
#   columns after each group is mapped
guess <- guessMapping(LTdataMapper, groups=groups, subset=subsets)
guess
```

Since we want out `LongTable` to have drugs as rows and samples as columns,
we see that both `justDrug` and `drugsAndDoses` yield the same result. So we
do not yet prefer one after the other. Looking at `justCells` and 
`cellsAndBatches`, we see one column maps to both of them and therefore still
have no preference. For `assay1` however, we see that no columns mapped, while
`assay2` maps many of the rows.
...
`assayMap` for the `LongTableDataMapper` and specify the it in a list of 
`character` vectors, one for each assay. Since viability is the raw measurement
and the final two columns are summaries, we will assign them to two assays:
...
map them to the appriate slots of an `S4` object. In our case, we are mapping
from the merckDT to a `LongTable`.
...
`rowData` or `colData` pasted together with a ':'.
