...
##FIXME:: This should be implemented as an extension of the union generic
##  from BiocGenerics
...
#' @keywords internal
.patternSearch <- 
  function(x, y, f, guess, n, guess_residual, lower_bounds, upper_bounds, span, 
           precision, step, scale, family, trunc)
  {
    ...
    while (span > precision) {
      for (neighbour in seq_len(nrow(neighbours))) {
        ...
        if (neighbour %% 2 == 1) {
          neighbours[neighbour, dimension] <- pmin(guess[dimension] + span * 
                                                     step[dimension], 
                                                   upper_bounds[dimension])
        } else {
          neighbours[neighbour, dimension] <- pmax(guess[dimension] - span * 
                                                     step[dimension], 
...
#' @keywords internal
.residual <- 
  function (x, y, n, pars, f, scale = 0.07, 
...
## FIXME:: This function already exists as base::trimws? Is there any reason
